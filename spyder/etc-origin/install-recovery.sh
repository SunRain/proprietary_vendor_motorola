...
echoDebug()
{
    ...
    echo ""
    echo "    if ! applypatch -c ${recovery_type}:${recovery_device}:${header_size}:${header_sha1}; then"
...
if [ -f ${CONF_FILE} ]; then
    ...
    if ! applypatch -c ${recovery_type}:${recovery_device}:${header_size}:${header_sha1}; then
